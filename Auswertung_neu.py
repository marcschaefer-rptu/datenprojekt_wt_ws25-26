...
def spearmanKorrelationsKoeffizient(data1, data2):
    """Spearman-Rangkorrelationskoeffizient - Monotone Abhängigkeit zweier Variablen
    Formel: r_s = 1 - (6 * Σ(d_i^2)) / (n*(n^2 - 1))
    Wertebereich: -1 (negativ) bis +1 (positiv korreliert)
    Misst monotone Beziehungen, robust gegenüber Ausreißern"""
    if len(data1) != len(data2):
        raise ValueError("Daten müssen gleiche Länge haben")
    n = len(data1)
    rank1 = rankdata(data1)
    rank2 = rankdata(data2)
    d_squared = sum((r1 - r2)**2 for r1, r2 in zip(rank1, rank2))
    rs = 1 - (6 * d_squared) / (n * (n**2 - 1))
    return rs
...
def urlist(filePath, fileName, data):
    """Schreibt Urlisten (jetzt sortiert) in separate CSV-Dateien
    Eine Datei pro Spalte: Liste aller Werte in sortierter Reihenfolge
    ...
    for spalte_idx in range(len(data[0])):
        ...
        # Sammle Originalwerte ohne Umwandlung zu str, damit numerisch sortiert werden kann
        werte_raw = [row[spalte_idx] for row in data]
        def __sort_key(x):
            try:
                return float(x)
            except Exception:
                return str(x)
        werte = sorted(werte_raw, key=__sort_key)
        
        with open(ausgabe_pfad, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([fileName[spalte_idx]])
            for wert in werte:
                writer.writerow([str(wert)])
        
def ranglist(filePath, fileName, data):
    """Schreibt Ranglisten (sortierte Werte) in separate CSV-Dateien
    Eine Datei pro Spalte: Liste aller Werte aufsteigend sortiert
    Format: Spaltenname als Header, dann sortierte Werte untereinander"""
    for spalte_idx in range(len(data[0])):
        ausgabe_pfad = os.path.join(script_dir, filePath + fileName[spalte_idx] + '.csv')
        werte_raw = [row[spalte_idx] for row in data]
...
def boxWhiskerPlot(filePath, fileName, data, yLabel):
    ...
    for spalte_idx in range(len(data[0])):
        ...
        ausgabe_pfad = os.path.normpath(ausgabe_pfad)
        plt.savefig(ausgabe_pfad, dpi=1147, bbox_inches='tight', facecolor='white')
        plt.close()

def histogramPlot(filePath, fileName, data, yLabel):
    """Erstellt Histogramme oder Balkendiagramme für jede numerische Spalte
    Bei wenigen eindeutigen Werten (diskret): Balkendiagramm
    Bei vielen Werten (kontinuierlich): Histogramm"""
    for spalte_idx in range(len(data[0])):
        werte = [row[spalte_idx] for row in data]
        ...
        # Prüfe, ob diskret (wenige eindeutige Werte oder Jahr/Monat)
        unique_werte = sorted(set(werte))
        if len(unique_werte) <= 12 or spalte_idx in [0, 1]:  # Diskrete Daten oder Jahr/Monat: Balkendiagramm
            counts = [werte.count(u) for u in unique_werte]
            print(f"Debug {fileName[spalte_idx]}: unique {len(unique_werte)}, counts {counts[:5]}...")  # Erste 5
            
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.bar(unique_werte, counts, color='#951E2D', alpha=0.7, edgecolor='#2c2c2c')
            
            # Formatierung
            ax.set_title(fileName[spalte_idx], fontsize=14, fontweight='bold', pad=15)
            ax.set_xlabel(yLabel[spalte_idx], fontsize=12)
            ...
            # Speichere
            ausgabe_pfad = os.path.join(script_dir, filePath + 'histogram-' + fileName[spalte_idx] + '.jpg')
            plt.savefig(ausgabe_pfad, dpi=1147, bbox_inches='tight', facecolor='white')
            plt.close()
        else:  # Kontinuierliche Daten: Histogramm
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.hist(werte, bins=20, color='#951E2D', alpha=0.7, edgecolor='#2c2c2c')
...
def scatterPlot(fileName, data, title, yLabel):
    ...
    if len(data[0]) > 3:
        ...
        ax.scatter(zeit, werte2, color='#3498DB', s=80, alpha=0.7, edgecolors='#2c2c2c', linewidth=1, label='Uebernachtungen', zorder=3)
        ...
        print(f"Sinus functions Werte:{A1}*sin({B1}*x+{C1})+{D1}")
        fitted2 = sine_function(zeit, A2, B2, C2, D2)
        print(f"Sinus functions Werte:{A2}*sin({B2}*x+{C2})+{D2}")
    ...
    if is_dataset_2:
        ...
        # Speichere zusätzliches Diagramm
        ausgabe_pfad_line = os.path.join(script_dir, fileName.replace('.jpg', '-line.jpg'))
        plt.savefig(ausgabe_pfad_line, dpi=1147, bbox_inches='tight', facecolor='white')
        plt.close()

def scatterPlotNoLine(fileName, data, title, yLabel):
    """Erstellt Streudiagramm (Scatter Plot) ohne Trendkurven
    - Nur Datenpunkte dargestellt
    - Keine sinusförmigen Fittkurven
    - Zeigt reine zeitliche Entwicklung"""
    if not data:
        return
    ...
    # Falls zwei Wertereihen: Nur Punkte
    if len(data[0]) > 3:
        werte2 = np.array([row[3] / 1000000 for row in data])
        ax.scatter(zeit, werte2, color='#3498DB', s=80, alpha=0.7, edgecolors='#2c2c2c', linewidth=1, label='Uebernachtungen', zorder=3)
...
if len(data1Array) > 0:
    ...
    histogramPlot('output/dataset-1/', ['Jahr', 'Monat', 'Elektrizizaetserzeugung'], data1Array, ['', '', 'in TWh'])
    ...
    # Scatter-Plot ohne Trendkurven erstellen
    scatterPlotNoLine('output/dataset-1/scatterPlot-noLine.jpg', data1Array, 'Energieerzeugung aus Steinkohle nach Jahren', 'Energieerzeugung in TWh')
    ...
    # Spearman-Korrelation zwischen Jahren und durchschnittlicher Energieerzeugung
    jahres_daten = {}
    for row in data1Array:
        jahr = row[0]
        wert = row[2]
        if jahr not in jahres_daten:
            jahres_daten[jahr] = []
        jahres_daten[jahr].append(wert)
    
    print(f"Debug Datensatz 1: jahres_daten keys {list(jahres_daten.keys())}, counts {[len(v) for v in jahres_daten.values()]}")
    
    jahre = sorted(jahres_daten.keys())
    jahres_durchschnitte = [sum(jahres_daten[j]) / len(jahres_daten[j]) for j in jahre]
    
    spearman_wert = spearmanKorrelationsKoeffizient(jahre, jahres_durchschnitte)
    
    with open(output_file, 'a', encoding='utf-8') as f:
        f.write(f"\nSpearman-Korrelation zwischen Jahren und durchschnittlicher Energieerzeugung: {spearman_wert:.3f}\n")
...
if len(data2Array) > 0:
    ...
    # Scatter-Plot ohne Trendkurven erstellen
    scatterPlotNoLine('output/dataset-2/scatterPlot-noLine.jpg', data2Array, 'Beschäftige im Einzelhandel im Vergleich zu 2015 nach Jahren', 'Beschaeftigte prozentual zu 2015')
...
if len(data3Array) > 0 and len(data3Array[0]) > 3:
    ...
    # Scatter-Plot ohne Trendkurven erstellen
    scatterPlotNoLine('output/dataset-3/scatterPlot-noLine.jpg', data3Array, 'Ankuenfte und Uebernachtungen in Beherbergungsbetrieben nach Jahren', 'Anzahl in Mio')
